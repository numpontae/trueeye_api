...
class registrationRoute {
  ...
  getConsentDataFromOnetrust() {
    return async (req: Request, res: Response) => {
      let { national_id, passport, site } = req.query
      let repos = di.get('repos')
      let identifier = !_.isEmpty(national_id) && national_id != 'null' ? national_id : passport
      try {
        let test = axios({method: 'post',url:`http://10.105.10.29:1881/onetrust_consent_data`, data:  {identifier, site}})
  ...
  postConsentDataToOnetrust() {
    return async (req: Request, res: Response) => {
      ...
      let Hash = await CryptoJS.algo.SHA256.create();
          await Hash.update(JSON.stringify(national_id, consentData));
          let hashid = await Hash.finalize().toString();
      let repos = di.get("repos");
      try {
        let test = axios({method: 'post',url:`http://10.105.10.29:1881/onetrust_consent_post`, data:  {national_id, site, consentData, hashid}})
      .then(function (response) {
        res.send({status: 200})
      }).catch(function (error) {
        res.send({status: 404})
