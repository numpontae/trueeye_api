...
class ctRoute {
  ...
  getTest() {
    return async (req: Request, res: Response) => {
      ...
      let dataRowID: any = await new Promise((resolve, reject) => {
          repos.reserve((err : any, connObj : any) => {
              if (connObj) {
                  let conn = connObj.conn;
                      ...
                      if (err) {
                          ...
                          statement.setFetchSize(100, function (err: any) {
                              if (err) {
                                  ...
                                  let query = `select PAPMI_No "HN", PAPMI_RowId1 from PA_PatMas Where PAPMI_No = '1100042101' `;
      ...
      dataRowID.map(async(d: any) => {
        let dataADM: any = await new Promise((resolve, reject) => {
          repos.reserve((err : any, connObj : any) => {
              if (connObj) {
                  let conn = connObj.conn;
                      ...
                      if (err) {
                          ...
                          statement.setFetchSize(100, function (err: any) {
                              if (err) {
                                  ...
                                  let query = `select '${d.HN}' "HN", PAADM_RowID from PA_Adm Where PAADM_PAPMI_DR = ${d.PAPMI_RowId1} `;
                      ...
                      if (err) {
                          ...
                          statement.setFetchSize(100, function (err: any) {
                              if (err) {
                                  ...
                                  let query = `CALL Custom_THSV_Report_ZEN_StoredProc.SVNHRptEprLabResult_GetData('${d.HN}', '${d.PAADM_RowID}') `;
