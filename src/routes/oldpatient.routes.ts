    ...
    async postPatientFamilyByHN(HN : any) {
        ...
        result.map(async (d : any) => {
            ...
            if (dataPerson.length > 0 && dataIllness.length > 0) {
                ...
                valuesFamily = {
                    HN: d.HN, Person: dataPerson[0].ID,
        ...
        return async (req : Request, res : Response) => {
            ...
            try {
                ...
                if (_.isEmpty(id) && !_.isNumber(id)) {
                    ...
                    query += ` WHERE 1 = 1`
                    if (!_.isEmpty(firstname)) {
                        query += ` AND (PI.Firstname = '${firstname}')`
                    }
                    if (!_.isEmpty(lastname)) {
                        query += ` AND (PI.Lastname = '${lastname}')`
                    }
                    ...
                    if (!_.isEmpty(firstname)) {
                        queryCount += ` AND (PI.Firstname = '${firstname}')`
                    }
                    if (!_.isEmpty(lastname)) {
                        queryCount += ` AND (PI.Lastname = '${lastname}')`
                    }
                    if (!_.isEmpty(phone_no)) {
...
const router = Router();
const route = new oldpatientgRoute();
...
export const oldpatient = router;
