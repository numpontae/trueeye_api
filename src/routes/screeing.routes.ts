...
class screeningRoute {
    async getPatientByHN(hn : any) {
        ...
        if (info[0].Type == 0) {
            ...
            let queryPatientSignature = `SELECT Signature, Createdate, Createtime FROM Screening.Signature Where HN = '${
                info[0].HN
            }' And SignType = 'Patient' Order By ID Desc`
            let queryStaffSignature = `SELECT Signature, Createdate, Createtime FROM Screening.Signature Where HN = '${
                info[0].HN
    ...
    async postScreeningFromTC(national_id : any, passport : any, firstname : any, lastname : any, HN : any) {
        ...
        await Promise.all(dataHN.map(async (d : any) : Promise<any> => {
            let HN = await this.postPatientInfoBYHN(d.HN)
            if (HN && HN != null)
            {
                if (d.NationalID) {
    ...
    async postPatientInfoBYHN(HN : any) {
        ...
        let result: any = await new Promise((resolve, reject) => {
            repos.reserve((err : any, connObj : any) => {
                if (connObj) {
                  ...
                  PAPMI_Title_DR->TTL_RowId "Title",
                  PAPMI_Name "Firstname",
                  PAPMI_Name8 "Middlename",
                  PAPMI_Name2 "Lastname",
                  PAPMI_Name3 "TitleEN",
                  PAPMI_Name5 "FirstnameEn",
                  PAPMI_Name6 "MiddlenameEN",
                  PAPMI_Name7 "LastnameEn",
                  tochar(PAPER_Dob, 'YYYY-MM-DD') "DOB",
                  PAPMI_Sex_DR "Gender",
                  PAPER_Nation_DR "Nationality",
                ...
                PAPER_PrefLanguage_DR "PreferredLanguage",
                  ...
                  SUBSTRING(PAPER_Marital_DR->CTMAR_Desc, CHARINDEX('(', PAPER_Marital_DR->CTMAR_Desc)+1, (LENGTH(PAPER_Marital_DR->CTMAR_Desc)-CHARINDEX('(', PAPER_Marital_DR->CTMAR_Desc))-1) "MaritalStatus",
                  PAPER_SocialStatus_DR->SS_Desc "Occupation",
                  PAPER_MobPhone "Mobilephone",
                  PAPER_TelH "Telephone" ,
            CASE
        ...
        if (result.length > 0) {
            let results = null
            repos = di.get("repos")
            let query = `SELECT HN FROM Screening.Patient_Info WHERE HN = '${result[0].HN}' AND 
                    Updatedate > '${
                result[0].DateOfEntry
            }'`

            results = await repos.query(query)


            if (results.length == 0) {
                let query_1 = `REPLACE INTO Screening.Patient_Info SET ? `
                repos.query(query_1, result[0], function (err: any, results: any) {
                    if (err) {
                        return console.error(err.message);
                    }
                });
    ...
    async updateScreeningDataGroupBYData(HN : any) {
        let repos = di.get("repos")
        let query = `REPLACE INTO Screening.Patient_Financial (HN) VALUES ('${HN}') `
        repos.query(query, function (err: any, results: any) {
            if (err) {
                return console.error(err.message);
            }
                        ...
                        if (err) {
                            ...
                            statement.setFetchSize(100, function (err: any) {
                                if (err) {
                                    ...
                                    const query = `SELECT TOP 1	NOK_PAPMI_ParRef->PAPMI_No "HN",
                              CASE 
                              ...
                              END "Province",
                                        ...
                                        AND NOK_PAPMI_ParRef->PAPMI_RowId->PAPER_AgeYr > 15
                                        AND NOK_ContactType_DR->CONTTP_Desc = 'Emergency Contact' 
            ...
            repos.reserve((err : any, connObj : any) => {
                if (connObj) {
                    ...
                    CASE 
                      ...
                      END "District",
                      PAPER_CityArea_DR "Subdistrict",
                      PAPER_StNameLine1 "Address",
                      PAPER_StName "FullAddress",
                            '0' "Type"
                      FROM PA_PatMas
                        INNER JOIN PA_Person ON PA_PatMas.PAPMI_PAPER_DR = PA_Person.PAPER_RowId
                        WHERE PAPMI_No = '${HN}'
    ...
    async postPatientFamilyByHN(HN : any) {
        ...
        result.map(async (d : any) => {
            ...
            let queryCheckFamily = `SELECT * FROM Registration.CT_Relation WHERE (ID like '%${
                d.Person
            }%' OR ID = '${
                d.Person
            }') And ActiveFamilyHistory = 1 `
            ...
            let valuesFamily = null
            if (dataPerson.length > 0 && dataIllness.length > 0) {

                valuesFamily = {
                    HN: d.HN,
                    Person: dataPerson[0].ID,
                    Disease: dataIllness[0].ID
                }
                repos.query(query_1, valuesFamily, function (err: any, results: any) {
                    if (err) {
                        return console.error(err.message);
                    }
        ...
        return async (req : Request, res : Response) => {
            ...
            try {
                ...
                if (_.isEmpty(id) && !_.isNumber(id)) {
                    ...
                    query += ` WHERE 1 = 1`
                    if (!_.isEmpty(firstname)) {
                        query += ` AND (PI.Firstname like '${firstname}%' OR PI.FirstnameEN like '${firstname}%')`
                    }
                    if (!_.isEmpty(lastname)) {
                        query += ` AND (PI.Lastname like '${lastname}%' OR PI.LastnameEN like '${lastname}%')`
                    }
                    ...
                    if (!_.isEmpty(hn)) {
                        query += ` AND (REPLACE(PI.HN,'-','') = REPLACE('${hn}','-',''))`
                    }

                    query += ` ORDER BY Updatedate DESC , Updatetime DESC LIMIT ${startNum},${LimitNum}`
                    let queryCount = `SELECT COUNT(PI.HN) as count FROM Screening.Patient_Info PI`
                    queryCount += ` WHERE 1 = 1`
                    if (!_.isEmpty(firstname)) {
                        queryCount += ` AND (PI.Firstname like '${firstname}%' OR PI.FirstnameEN like '${firstname}%')`
                    }
                    if (!_.isEmpty(lastname)) {
                        queryCount += ` AND (PI.Lastname like '${lastname}%' OR PI.LastnameEN like '${lastname}%')`
                    }
                    if (!_.isEmpty(phone_no)) {
                    ...
                    if (!_.isEmpty(hn)) {
                        queryCount += ` AND (REPLACE(PI.HN,'-','') = REPLACE('${hn}','-',''))`
                    }
                    queryCount += ` ORDER BY Updatedate DESC , Updatetime DESC`
                    // let data
                    // let count
                    ...
                    if (data.length == 0) {
                        ...
                        await this.postScreeningFromTC(national_id, passport, firstname, lastname, hn)
    ...
    async updateAdult(body : any) {
        ...
        let queryInfo = `UPDATE Screening.Patient_Info SET ? WHERE HN = '${
            body.HN
        }'`
        ...
        let queryPresent = `UPDATE Screening.Patient_Address SET ? WHERE HN = '${
            body.HN
        }' And Type = 0`
        ...
        let queryPermanent = `UPDATE Screening.Patient_Address SET ? WHERE HN = '${
            body.HN
        }' And Type = 1`
        ...
        let queryEmergency = `UPDATE Screening.Patient_Emergency SET ? WHERE HN = '${
            body.HN
        }'`
        ...
        let queryFinancial = `UPDATE Screening.Patient_Financial SET ? WHERE HN = '${
            body.HN
        }'`
        let dataHistory = {
        ...
        let queryHistory = `UPDATE Screening.Patient_History SET ? Where HN = '${
            body.HN
        }'`
        ...
        let deleteFamily = `DELETE FROM Screening.Family_History WHERE HN = '${
            body.HN
        }'`
        ...
        let deleteSocial = `DELETE FROM Screening.Patient_Social WHERE HN = '${
            body.HN
        }'`
    ...
    async updateChild(body : any) {
        ...
        let queryInfo = `UPDATE Screening.Screening SET ? WHERE HN = '${
            body.HN
        }'`
        ...
        let queryPediatric = `UPDATE Screening.Pediatric SET ? WHERE HN = '${
            body.HN
        }'`
        ...
        let deleteParent = `DELETE FROM Screening.Parent WHERE HN = '${
            body.HN
        }'`
    ...
    async handleConsent(data : any, hn : any) {
        let repos = di.get("repos")
        let query_1 = `REPLACE INTO Screening.Consent (HN, Clause, Message, Agree) VALUES('${hn}', '1', '${
            data.consent_1.text
        }', ${
            data.consent_1.status
        })`
        let query_2_1 = `REPLACE INTO Screening.Consent (HN, Clause, Message, Agree) VALUES('${hn}', '2_1', '${
            data.consent_2_1.text
        }', ${
            data.consent_2_1.status
        })`
        let query_2_2 = `REPLACE INTO Screening.Consent (HN, Clause, Message, Agree) VALUES('${hn}', '2_2', '${
            data.consent_2_2.text
        }', ${
            data.consent_2_2.status
        })`
        let query_2_3 = `REPLACE INTO Screening.Consent (HN, Clause, Message, Agree) VALUES('${hn}', '2_3', '${
            data.consent_2_3.text
        }', ${
            data.consent_2_3.status
        })`
        let query_2_4 = `REPLACE INTO Screening.Consent (HN, Clause, Message, Agree) VALUES('${hn}', '2_4', '${
            data.consent_2_4.text
        }', ${
            data.consent_2_4.status
        })`
        let query_2_5_1 = `REPLACE INTO Screening.Consent (HN, Clause, Message, Agree) VALUES('${hn}', '2_5_1', '${
            data.consent_2_5_1.text
        }', ${
            data.consent_2_5_1.status
        })`
        let query_2_5_2 = `REPLACE INTO Screening.Consent (HN, Clause, Message, Agree) VALUES('${hn}', '2_5_2', '${
            data.consent_2_5_2.text
        }', ${
            data.consent_2_5_2.status
        })`
        let query_2_5_3 = `REPLACE INTO Screening.Consent (HN, Clause, Message, Agree) VALUES('${hn}', '2_5_3', '${
        ...
        if (data.other.choice_1.status === true) {
            let query = `REPLACE INTO Screening.Consent (HN, Clause, Message, Agree) VALUES(${hn}, 'other_1', '${
        ...
        if (data.other.choice_2.status === true) {
            let query = `REPLACE INTO Screening.Consent (HN, Clause, Message, Agree) VALUES(${hn}, 'other_2', '${
        ...
        if (data.other.choice_3.status === true) {
            let query = `REPLACE INTO Screening.Consent (HN, Clause, Message, Agree) VALUES(${hn}, 'other_3', '${
        ...
        if (data.other.choice_4.status === true) {
            let query = `REPLACE INTO Screening.Consent (HN, Clause, Message, Agree) VALUES(${hn}, 'other_4', '${
    ...
    async approveAdult(body : any) {
        ...
        let dataInfo = {
            ...
            Passport: body.patient_info.passport,
            Occupation: body.patient_info.occupation,
            Mobilephone: body.patient_info.mobilephone,
            Telephone: body.patient_info.telephone,
            Email: body.patient_info.email,
            Homephone: body.patient_info.homephone,
            Officephone: body.patient_info.officephone,
            DefaultLanguage: body.defaultlanguage
        ...
        let dataPermanent = {
            ...
            Address: body.permanent.sameAddress ? body.present.address : body.permanent.address,
            Province: body.permanent.sameAddress ? body.present.provinceid : body.permanent.provinceid,
            sameAddress: body.permanent.sameAddress ? "1" : "0"
        }
            ...
            if (body.type == 0) {
                ...
                let family = await Promise.all(body.personal_history.family.map(async (item : any) : Promise < any > => {
                    if (item.person != null && item.illness != null) {
                        let queryRelation = `SELECT * FROM Registration.CT_Relation Where ID = ${
                            item.person
                        }`
                        let relation = await repos.query(queryRelation)
                        return {
                ...
                if (body.personal_history.alcohol.status) 
                    await social.push(dataalcohol)
                ...
                if (body.personal_history.exercise.status) 
                    await social.push(dataexercise)
                ...
                if (body.personal_history.smoke.status) 
                    await social.push(datasmoke)
                ...
                let time = new Date();
                const filename = `RPA_Screening_Adult_${
                    time.getFullYear()
                }-${
                    ("0" + (
                        time.getMonth() + 1
    ...
    postPatientByHN() {
        return async (req : Request, res : Response) => {
            let {HN} = req.body
            if (!HN.includes("TMP")) {
                HN = await this.postPatientInfoBYHN(HN)
                console.log(HN)
                if (HN && HN != null) {
                    console.log(HN)
                    await this.postEmergencyByHN(HN)
                    await this.postParentByHN(HN)
                    await this.postPatientAddressByHN(HN)
                    await this.postPatientFamilyByHN(HN)
                    await this.postPatientSocialByHN(HN)
                    await this.updateScreeningDataGroupBYNationID(HN)
                }
...
router.post("/", route.postScreening()).post("/search", route.getSearch()).post("/signature", route.saveSignature()).post("/update", route.updateData()).post("/getPendingData", route.getPendingData()).post("/approve", route.approveData()).post("/screening", route.updateScreening()).post("/signatureApprove", route.saveSignatureApprove()).post("/getApprovedData", route.getApprovedData()).post("/postpatientbyhn", route.postPatientByHN())
